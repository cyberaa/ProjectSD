...
public class TransactionalTrading
{
	...
	public synchronized static void checkQueue(Connection db, int idea_id) throws SQLException
	{
		...
		try {
			...
			{
				try {
					int res = ServerRMI.transactions.buyShares(rs.getInt("user_id"), rs.getInt("idea_id"), rs.getInt("share_num"), rs.getDouble("price_per_share"), rs.getDouble("new_price_share"), true, null, null);
